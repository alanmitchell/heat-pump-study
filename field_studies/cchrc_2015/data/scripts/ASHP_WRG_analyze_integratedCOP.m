...
load('Wrangell_WRG_DHP_TenSecond_1min.mtl','-mat');

if 1 %Use 1 if you only want to take data after 12/2/2014 (when cleaning of filters and anemometer was done:
    x_start=datenum('"2014-12-03 00:00:00"','"yyyy-mm-dd HH:MM:SS"');
    x_start_i=find(x>x_start,1,'first');
    x=x(x_start_i:end);
    T_deliv=T_deliv(x_start_i:end);
    T_return=T_return(x_start_i:end);
    T_coil=T_coil(x_start_i:end);
    T_out=T_out(x_start_i:end);
    P_input=P_input(x_start_i:end);
    Airflow=Airflow(x_start_i:end);
...
for i = 1:length(x) %x represents the time axis
    defrost = (T_coil(i)>(T_out(i)+5)) & (P_input(i)>100); %Analyze the status of defrost
    ...
    if (defrost_previous==true) && (defrost==false) %if defrost ended, process the cycle
        if Airflow(i-1)>0 %if the defrost that just eneded wasn't a real defrost and it was just heat pump in minimum flow waiting for thermostat call, then don't count this cycle and neither next cycle (see 12/23 15:00 for such situation)
            ...
            if ((x(i)-x(i_start))*24<10) && ((x(i)-x(i_start))*24>0.2) %if the length of the cycle is too long or too short, then it is not a real cycle, so don't record it
                if (x(i)-x(i_defrost_start))*24*60<30 %if the length of the defrost is too long, then it is not a real defrost, so don't count the cycle
