...
load('Wrangell_WRG_DHP_TenSecond_5min.mtl','-mat');

logging_interval=(x(2)-x(1))*24; %Calculate the logging interval (in hours) from the first two time stamps


n=8; %n specifies how many samples next to each other will be analyzed for steady state
...
ss=ss' & (P_output>100); %Only take samples with output power above certain threshold
...
if 1 %Use "1" if this plot is done as a part of a larger plot that is combining plots from several programs
    plot(T_out,COP,'.r','MarkerSize',15);
else
