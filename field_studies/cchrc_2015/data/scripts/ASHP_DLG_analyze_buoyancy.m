...
file_name='CR1000_DLG_DHP_TenSecondShort2.dat'; %Specify the file from which you want to read data
...
%Studying the flow resistance due to buoyancy with fan running on high:
%AirflowHighI=Airflow>180; %indices of high air flow
%AirflowHigh=Airflow(AirflowHighI); %extract samples with high air flow
%T_returnHigh=T_return(AirflowHighI);
%T_delivHigh=T_deliv(AirflowHighI);
%figure;
%plot(T_delivHigh-T_returnHigh,AirflowHigh,'.','MarkerSize',1);
%xlabel('T_{deliv} - T_{return} [F]');
%ylabel('Airflow [cfm]');
%title('Flow resistance due to buoyancy; fan running on high');
