function dynamicDataDisplay(axH)
%DYNAMICDATADISPLAY is a function for the ASHP program. This function
...
% ------------ End of dynamicDataDisplay -----------------------
    ...
    function displayData(obj,ev,varargin)%This callback function displays performance characteristics (energy input, energy output, COP,etc.).
        ...
        global x logging_interval T_out P_input P_output Airspeed; %These were created in the ASHP program and now they are needed here.
        ...
        E_input=nansum(P_input(i1:i2))*logging_interval/1000; %Energy input in kWh
        E_output=nansum(P_output(i1:i2))*logging_interval/1000; %Energy output in kWh
        COP=E_output/E_input; %COP
        Avg_T_out=nanmean(T_out(i1:i2)); %Average outside temperature
        %Avg_Airspeed=nanmean(Airspeed(i1:i2));
