%This program does what ASHP_WRG does, but in addition to it:
...
if 0 %Use 1 to load a file in the Campbell datalogger format or use 0 to load a file in Matlab format
    ...
    %Replace wrong data (due to mulfunctioning multiplexor) with Nan:
    i_wrong = (T_deliv<0) | isnan(T_deliv);
    T_deliv(i_wrong)=NaN;
    T_return(i_wrong)=NaN;
    T_coil(i_wrong)=NaN;
    T_out(i_wrong)=NaN;
    
    %Calculate the airflow from the air speed:
    Airflow=1.1922*Airspeed-26.052; %Air flow in cfm from proxy calibration
    ...
    save('Wrangell_WRG_DHP_TenSecond_10min.mtl','x','T_deliv','T_return','T_coil','T_out','P_input','Airflow','P_output');
else
    load('Wrangell_WRG_DHP_TenSecond_10min.mtl','-mat');
end

%T_deliv(isnan(T_deliv))=-300; %Make NaN visible in graph
